...
export const logIn = (req, res) => {
  res.status(200).json({ msg: `Holis! ${req.body.username}` });
};
...
export const asignarCarrito = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      return res
        .status(404)
        .json({ ok: false, msg: "Erro al buscar el usuario" });
    }
  } catch (error) {
    logger.error(error);
  }
  ...
  try {
    ...
    if (user) {
      ...
      if (upd) {
        ...
        return res
          .status(401)
          .json({ ok: false, msg: "Error, al finalizar compra" });
      }
    } else {
      return res
        .status(404)
